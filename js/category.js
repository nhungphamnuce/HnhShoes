...
let catSelect = document.getElementById('categorySelect')  //đọc trong phần HTML có phần class là catSelect sau đó gán thành biến categorySelect
...
catSelect.onchange = function(){  //function thay đổi hiển thị
    // lựa chọn được select
    let selectId= this.value;
    for(let i=1;i<4;i++){
        let id="category"+i
        if(selectId==i){
            document.getElementById(id).style.display = "block" //hiện lên
        }
        else
